...
// Preload some heavy bits on fast networks
...
const VehicleDetails = () => {
  ...
  const { vehicle, isFavorite, galleryImages, monthlyEMI, toggleFavorite, navigate, isLoading, error } =
    useVehicleData();
  const { reportMetric } = useWebVitalsOptimized();
  const { isLowMemory } = useMemoryPressure();
  const { getMetricsSummary } = useCoreWebVitals();

  const { nextImage, previousImage } = useImageCarousel({
  ...
  React.useEffect(() => {
    ...
    reportMetric({ name: "vehicle-details-mount", value: 0, rating: "good", delta: 0 });
              ...
              {shouldRenderHeavyContent && (
                <Suspense fallback={<ComponentLoading />}>
                  <section id="related" className="py-8 lg:py-16 bg-muted/30" aria-labelledby="related-vehicles-heading">
                    <h2 id="related-vehicles-heading" className="sr-only">
                      Related Vehicles
                    </h2>
