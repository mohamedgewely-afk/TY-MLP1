...
const InteractiveSpecsTech: React.FC<InteractiveSpecsTechProps> = ({ vehicle }) => {
  ...
  const getPerformanceSpecsForGrade = (grade: string) => {
    const baseSpecs = [
      {
        category: "Safety",
        ...
        specs: grade === "Limited" ? [
          { label: "Pre-Collision System", value: "Advanced with Pedestrian Detection", icon: <Car className="h-4 w-4" /> },
          ...
          { label: "Automatic High Beams", value: "Adaptive LED", icon: <Zap className="h-4 w-4" /> },
          ...
          { label: "Lane Departure Alert", value: "Basic", icon: <Navigation className="h-4 w-4" /> },
          { label: "Dynamic Radar Cruise Control", value: "Standard Range", icon: <Gauge className="h-4 w-4" /> },
          { label: "Automatic High Beams", value: "LED", icon: <Zap className="h-4 w-4" /> },
          { label: "Blind Spot Monitor", value: "Optional", icon: <Shield className="h-4 w-4" /> }
        ]
      },
      {
        category: "Technology",
        ...
        specs: grade === "Limited" ? [
          { label: "Infotainment Display", value: "14-inch Premium Touchscreen", icon: <Monitor className="h-4 w-4" /> },
          { label: "Wireless Connectivity", value: "Apple CarPlay & Android Auto", icon: <Wifi className="h-4 w-4" /> },
          { label: "Premium Audio", value: "12-Speaker JBL System", icon: <Volume2 className="h-4 w-4" /> },
          ...
          { label: "Navigation System", value: "Standard Navigation", icon: <MapPin className="h-4 w-4" /> },
          { label: "Remote Connect", value: "Basic App Integration", icon: <Smartphone className="h-4 w-4" /> }
        ]
      },
      {
        ...
        specs: selectedEngine === "Hybrid 2.5L" ? [
          ...
          { label: "Engine Type", value: "Naturally Aspirated", icon: <Car className="h-4 w-4" /> },
          { label: "Total Power Output", value: "203 HP", icon: <Gauge className="h-4 w-4" /> },
          { label: "Fuel Economy", value: "15.8 km/L City", icon: <Fuel className="h-4 w-4" /> },
          { label: "Acceleration", value: "0-100 km/h in 9.1s", icon: <Car className="h-4 w-4" /> },
          { label: "Drive Modes", value: "Eco, Normal, Sport", icon: <Settings className="h-4 w-4" /> }
        ]
      }
    ];
    return baseSpecs;
  ...
  const performanceSpecs = getPerformanceSpecsForGrade(selectedGrade);
  const availableGrades = getGradesForEngine(selectedEngine);

  // Ensure selected grade is available for current engine
  React.useEffect(() => {
    if (!availableGrades.find(g => g.name === selectedGrade)) {
      setSelectedGrade(availableGrades[0].name);
    }
  }, [selectedEngine]);
  ...
  return (
    <section className="py-8 lg:py-16 bg-gradient-to-br from-muted/20 to-background">
      <div className="toyota-container">
        ...
        {/* Performance Specs Carousel - Changes based on Grade */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        >
          ...
          <div className="relative max-w-4xl mx-auto">
            ...
            <AnimatePresence mode="wait">
              <motion.div
                key={`${currentPerformanceIndex}-${selectedGrade}-${selectedEngine}`}
