...
import { 
  ChevronLeft, ChevronRight, Users, Coffee, Car, Heart, Star,
  Mountain, Waves
} from "lucide-react";
...
const EnhancedLifestyleGallery: React.FC<EnhancedLifestyleGalleryProps> = ({ vehicle }) => {
  ...
  const lifestyleScenarios = [
    ...
    },
    {
      ...
      icon: <Coffee className="h-5 w-5 md:h-6 md:w-6" />,
    ...
    },
    {
      ...
      icon: <Waves className="h-5 w-5 md:h-6 md:w-6" />,
    ...
    },
    {
      ...
      icon: <Mountain className="h-5 w-5 md:h-6 md:w-6" />,
  ...
  return (
    <section className="py-8 md:py-16 lg:py-24 bg-gradient-to-br from-background to-muted/30 relative overflow-hidden">
      <div className="toyota-container relative z-10 px-4 md:px-6">
        ...
        {/* Interactive Lifestyle Experience - Mobile First */}
        <div className="relative" ref={swipeableRef}>
          <AnimatePresence mode="wait">
            ...
            >
              {/* Lifestyle Image - Mobile Optimized */}
              <div className="relative overflow-hidden rounded-2xl md:rounded-3xl">
                ...
                <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 md:hidden">
                  <div className="bg-white/90 backdrop-blur-lg rounded-full px-4 py-2 border border-white/20">
              ...
              {/* Lifestyle Details - Stacked Layout for Mobile */}
              <div className="space-y-6 md:space-y-8">
                <div className="space-y-4 md:space-y-6">
                  <div className="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
                    <div className={`w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-gradient-to-br ${currentScenario.color} flex items-center justify-center text-white shadow-lg md:shadow-2xl flex-shrink-0`}>
                      {currentScenario.icon}
                    </div>
                    <div className="text-center sm:text-left">
                      <h3 className="text-xl md:text-3xl lg:text-4xl font-bold">{currentScenario.title}</h3>
                      <p className="text-muted-foreground text-sm md:text-lg">{currentScenario.description}</p>
                    </div>
                  </div>
                  ...
                  <Card className="bg-gradient-to-br from-card to-muted/30 border-0 shadow-lg">
                    <CardContent className="p-4 md:p-6">
                      <h4 className="font-bold mb-3 md:mb-4 flex items-center text-sm md:text-base">
                        <Star className="h-4 w-4 md:h-5 md:w-5 mr-2 text-primary" />
                      ...
                      <div className="space-y-2 md:space-y-3">
                        {currentScenario.features.map((feature, idx) => (
                          <motion.div
                            ...
                            className="flex items-center space-x-3 p-2 md:p-3 bg-background/60 rounded-lg md:rounded-xl"
                          >
          ...
          <button
            ...
            className="absolute left-2 md:left-4 top-32 md:top-40 p-3 md:p-2 rounded-full bg-white/90 shadow-md border transition-all hover:bg-white hover:shadow-lg hover:scale-110 z-10"
            style={{ minWidth: '44px', minHeight: '44px' }}
          >
            <ChevronLeft className="h-5 w-5 md:h-4 md:w-4 text-gray-700" />
          </button>

          <button
            onClick={() => setSelectedScenario(prev => prev < lifestyleScenarios.length - 1 ? prev + 1 : 0)}
            className="absolute right-2 md:right-4 top-32 md:top-40 p-3 md:p-2 rounded-full bg-white/90 shadow-md border transition-all hover:bg-white hover:shadow-lg hover:scale-110 z-10"
            style={{ minWidth: '44px', minHeight: '44px' }}
          >
            <ChevronRight className="h-5 w-5 md:h-4 md:w-4 text-gray-700" />
          </button>
        </div>

        {/* Scenario Navigation - Mobile Optimized */}
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
            {lifestyleScenarios.map((scenario, index) => (
              <motion.button
                ...
                className={`p-4 md:p-4 rounded-xl md:rounded-2xl text-left transition-all duration-300 ${
              ...
              >
                <div className="flex items-center space-x-3 mb-2">
                  {scenario.icon}
                  <h4 className="font-bold text-xs md:text-sm">{scenario.title}</h4>
                </div>
