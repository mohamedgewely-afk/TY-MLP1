...
import {
  Check, Zap, Fuel, Settings, ArrowUpDown, Star, Car, Wrench, X
} from "lucide-react";
...
const VehicleConfiguration: React.FC<VehicleConfigurationProps> = ({
  ...
  const [compareSet, setCompareSet] = useState<number[]>([]); // indices of selected grades to compare
  ...
  const handleEngineChange = useCallback(
    async (engineName: string) => {
      ...
      setSelectedGrade(0);
      setCompareSet([]); // reset compare on engine switch
      setAnimationKey((prev) => prev + 1);

      await new Promise((resolve) =>
        setTimeout(resolve, performanceConfig.animations.duration * 1000)
      );
  ...
  // Do not render heavy UI until in viewport
  ...
  return (
    <>
      <section
        ref={targetRef}
        className="py-8 lg:py-16 bg-gradient-to-br from-background via-muted/20 to-background"
      >
        <div className="toyota-container">
          ...
          >
            ...
            <div
              className={`${
                isMobile ? "grid grid-cols-2 gap-2 px-4" : "grid md:grid-cols-2 gap-6"
              } max-w-4xl mx-auto`}
            >
              {engines.map((engine) => (
                <motion.div
                  key={engine.name}
                  whileHover={
                    performanceConfig.animations.enabled
                ...
                >
                  ...
                  >
                    <CardContent className={isMobile ? "p-2" : "px-6 py-4"}>
                        ...
                        <AnimatePresence>
                          {engine.selected && (
                            <motion.div
                              ...
                              transition={{
                                duration: performanceConfig.animations.duration,
                              }}
                              className={`${
                      ...
                      <h4
                        className={`${
                          isMobile ? "text-xs" : "text-lg lg:text-xl"
                        } font-bold mb-1`}
                      >
            ...
            <AnimatePresence mode="wait">
              {isGradeLoading ? (
                ...
                <motion.div
                  ...
                  transition={{
                    duration: performanceConfig.animations.duration * 1.5,
                  }}
                >
                  {/* Desktop: 3 tiles grid */}
                  {!isMobile && (
                    <motion.div
                            ...
                            >
                              <CardContent className="p-0">
                                <div className="relative overflow-hidden">
                                  {grade.badge && (
                                    <Badge
                                      className={`absolute top-4 left-4 z-10 ${grade.badgeColor} text-white px-3 py-1 text-sm font-medium`}
                                    >
                                      {grade.badge}
                                    </Badge>
                                  )}
                                  ...
                                  <button
                                    ...
                                    className={`absolute top-4 right-4 z-10 rounded-full px-3 h-8 text-xs border ${
                                      isCompared
                                ...
                                <div className="p-6 space-y-4">
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    {/* Keep extra clean; show monthly if you want */}
                                    {/* <div className="text-sm text-muted-foreground">
                                      From AED {grade.monthlyFrom}/month
                                    </div> */}
                                  </div>

                                  {/* Exactly TWO bullets for cleanliness */}
                  ...
                  {/* Mobile: snap carousel (no external deps) */}
                  {isMobile && (
                    <div className="px-4">
                              ...
                              >
                                <CardContent className="p-0">
                                  <div className="relative">
                                    {grade.badge && (
                                      <Badge
                                        className={`absolute top-3 left-3 z-10 ${grade.badgeColor} text-white px-2 py-0.5 text-[10px]`}
                                      >
                                        {grade.badge}
                                      </Badge>
                                    )}
                                    <button
                                      ...
                                      className={`absolute top-3 right-3 z-10 rounded-full px-2 h-7 text-[11px] border ${
                                        isCompared
                                          ? "bg-primary text-primary-foreground border-primary"
                                          : "bg-background/90 border-border"
                                      }`}
          ...
          <div className="mx-auto max-w-6xl rounded-2xl border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 shadow-lg p-3">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-sm font-medium mr-1">
                {compareSet.length} grades selected
              </span>
      ...
      {/* Compare Modal (uses your existing component) */}
