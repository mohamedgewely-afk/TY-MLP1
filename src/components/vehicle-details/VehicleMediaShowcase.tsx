    ...
    gallery: [
      {
        ...
        details: {
          overview: "Ergonomics tuned for clarity, reach, and minimal eye-off-road.",
...
const VARIANT: Record<
  MediaItem["id"],
  { accent: string; slab: string; chips: string }
> = {
  v6: { accent: "text-red-600", slab: "bg-red-50/70", chips: "bg-red-100" },
  interior: { accent: "text-amber-700", slab: "bg-amber-50/70", chips: "bg-amber-100" },
  safety: { accent: "text-blue-700", slab: "bg-blue-50/70", chips: "bg-blue-100" },
  handling: { accent: "text-emerald-700", slab: "bg-emerald-50/70", chips: "bg-emerald-100" },
  quality: { accent: "text-zinc-700", slab: "bg-zinc-50/70", chips: "bg-zinc-100" },
  connect: { accent: "text-indigo-700", slab: "bg-indigo-50/70", chips: "bg-indigo-100" },
};

/* ---------------------------------- Props ---------------------------------- */
interface Props {
  vehicle: VehicleModel; // kept for compatibility with the calling page
}
...
const VehicleMediaShowcase: React.FC<Props> = () => {
  ...
  /* bridge CTA — do not touch VehicleDetails.tsx */
  const openBooking = () => {
    try {
      window.dispatchEvent(
        new CustomEvent("open-booking", { detail: { source: "VehicleMediaShowcase" } })
      );
  ...
  return (
    <section className={cx(TOK.max, "px-4 md:px-6")}>
      {/* Video (contained on desktop) */}
      ...
      <div className="mb-6 md:hidden">
        <div
          ref={mobWrapRef}
          className="no-scrollbar -mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4"
        >
          {items.map((m) => (
            <button
              key={m.id}
              ...
              className={cx(
                ...
                TOK.radius,
                TOK.ring,
                "snap-start min-w-[86%] overflow-hidden text-left"
              )}
            >
              <div className="relative">
                ...
                />
                <div
                  className="absolute left-0 top-0 h-1 rounded-tl-[16px]"
                  style={{ background: TOK.red, width: 70 }}
                />
              </div>
              <div className="p-4">
                <div className="mb-1 flex items-center gap-2">
                  ...
                  {m.badges?.slice(0, 2).map((b) => (
                    <span
                      key={b}
                      className="rounded-full border border-zinc-200 px-2 py-0.5 text-[11px]"
                    >
                      {b}
                    </span>
                  ))}
        ...
        {items.map((m) => (
          <button
            ...
            className={cx(
              ...
              TOK.radius,
              TOK.ring,
              "overflow-hidden text-left transition-shadow hover:shadow-md"
            )}
          >
            <div className="relative">
              <ImageSafe src={m.thumbnail} alt={m.title} cover className="h-56 w-full md:h-64" />
              <div
                className="absolute left-0 top-0 h-1 rounded-tl-[16px]"
                style={{ background: TOK.red, width: 82 }}
              />
            </div>
            <div className="p-4">
              <div className="mb-1 flex items-center gap-2">
      ...
      {open && (
        ...
        >
          <div
            className={cx(
              "bg-white",
              "h-[100svh] w-full md:h-[92vh] md:max-w-[1300px]",
              "md:rounded-2xl",
              "overflow-hidden"
            )}
          ...
          >
            ...
            <div className="sticky top-0 z-10 border-b bg-white/95 px-3 py-3 backdrop-blur md:px-6">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={openBooking}
                    className="hidden rounded-full px-3 py-2 text-xs font-semibold text-white md:inline-flex"
            ...
            {/* Body: no outer scroll — fixed grid */}
            <div className="grid h-[calc(100svh-57px)] md:h-[calc(92vh-57px)] grid-rows-[minmax(0,1fr)_auto] md:grid-cols-[minmax(0,7fr)_minmax(0,5fr)] md:grid-rows-1">
              {/* Visual */}
              <div
              ...
              >
                {visualIsVideo ? (
                  ...
                  <ImageSafe
                    src={currSlide?.url || open.thumbnail}
                    alt={currSlide?.title || open.title}
                    cover
              ...
              {/* Content — tabbed, no outer scroll */}
              <div className="flex min-h-0 flex-col bg-white md:rounded-r-2xl">
                    ...
                    {(["overview", "specs", "features", "tech"] as const).map((t) => (
                      <button
                        ...
                        className={cx(
                          ...
                          tab === t ? "border-zinc-900" : "border-zinc-300"
                        )}
                      >
                ...
                {/* Panels (switch, no scroll) */}
                <div className="grid flex-1 grid-rows-[auto_1fr] gap-4 p-4">
                  <div
                    className={cx(
                      "rounded-xl p-4",
                      VARIANT[open.id].slab,
                      "border border-zinc-200/60"
                    )}
                  >
                    <h5 className={cx("mb-1 text-lg font-semibold", VARIANT[open.id].accent)}>
                  ...
                  {/* grid that fits without scrolling */}
                    ...
                    {tab !== "overview" && (
                      <>
                        <SpecCard
                          ...
                          accentClass={VARIANT[open.id].accent}
                        />
                        {/* keep the layout dense with mirrored cards */}
                        <SpecCard
                          title="Key Points"
                          bullets={(currSlide?.details?.overview
                            ? [currSlide.details.overview]
                            : open.badges) as string[]}
                          accentClass={VARIANT[open.id].accent}
                        />
                        <SpecCard
                          title="Related"
