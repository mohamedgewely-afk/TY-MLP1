...
// Premium Toyota "Journey" media studio (no deps). Desktop = editorial split with rich content.
// Mobile = true full-screen, edge-to-edge media, vertical content (no horizontal rails).
...
const card: CSSProperties = { background: COLOR.bg, border: `1px solid ${COLOR.line}`, borderRadius: R.card, boxShadow: "0 1px 2px rgba(15,23,42,.06)" };
...
const tag: CSSProperties = { display: "inline-flex", alignItems: "center", borderRadius: 999, padding: "6px 10px", fontSize: 12, border: `1px solid ${COLOR.line}`, background: "#fff", color: COLOR.ink };
...
/* ----- Media widgets ----- */
const SafeImage: React.FC<{ src?: string; alt?: string; fit?: "cover" | "contain"; aspect?: number; style?: CSSProperties }> = ({
  src, alt, fit = "contain", aspect = 16 / 9, style
}) => {
  ...
  return (
    <div style={{ position: "relative", width: "100%", aspectRatio: String(aspect), background: fit === "contain" ? "#000" : "#111", overflow: "hidden", ...style }}>
      {!err ? (
        ...
        <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", color: "#9CA3AF", background: "#F3F4F6", fontSize: 12 }}>Image unavailable</div>
      )}
    </div>
  );
};
...
const WistiaVideo: React.FC<{ id: string; autoPlay?: boolean; muted?: boolean; aspect?: number; style?: CSSProperties }> = ({
  ...
  const qs = new URLSearchParams({
    seo: "false", dnt: "1", playsinline: "1", autoplay: autoPlay ? "1" : "0", muted: m ? "1" : "0",
    controlsVisibleOnLoad: "true", playbar: "true", fullscreenButton: "true", volumeControl: "true"
  });
  ...
  return (
    <div style={{ position: "relative", width: "100%", aspectRatio: String(aspect), borderRadius: R.card, overflow: "hidden", ...style }}>
      ...
      {m && autoPlay && (
        <button onClick={() => setM(false)} style={{ ...btn, position: "absolute", right: 12, top: 12, background: COLOR.ghost }}>Unmute</button>
      )}
    </div>
  );
};
    ...
    journey: [
      {
        ...
        bullets: ["Dual circuits", "Low-temp charge-air", "Repeatable lap pace"],
      ...
      },
      {
        ...
        title: "On-Road Pull",
        body: "Watch torque delivery & shift logic in sync.",
...
const Tile: React.FC<{ item: MediaItem; onClick: (m: MediaItem) => void; big?: boolean }> = ({ item, onClick, big }) => {
  return (
    <div onClick={() => onClick(item)} style={{ ...card, overflow: "hidden", cursor: "pointer", transition: "transform .2s ease" }}
...
/* ----- Rich content widgets for the modal ----- */
const StatPill: React.FC<{ label: string }> = ({ label }) => (
  <span style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 999, border: `1px solid ${COLOR.line}`, background: "#fff", fontSize: 12 }}>
...
const BulletBlock: React.FC<{ title: string; items?: string[]; dotColor?: string }> = ({ title, items, dotColor = "#10B981" }) => {
  if (!items || items.length === 0) return null;
  return (
    <div style={{ ...card, padding: 16 }}>
      ...
      <ul>
        {items.map((b, i) => (
          <li key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: i === items.length - 1 ? "0" : `1px dashed ${COLOR.line}` }}>
...
/* ----- Journey Modal (rich) ----- */
const JourneyModal: React.FC<{ open: boolean; item?: MediaItem; onClose: () => void }> = ({ open, item, onClose }) => {
  ...
  const begin = (x: number) => { startX.current = x; setDragX(0); };
  const move = (x: number) => { if (startX.current !== null) setDragX(x - startX.current); };
  const end = () => {
    const THRESH = 60;
  ...
  const head: CSSProperties = { display: "flex", alignItems: "center", justifyContent: "space-between", padding: isDesktop ? "12px 16px" : "14px 16px", borderBottom: `1px solid ${COLOR.line}`, background: "#fff" };

  // Content grid: media left, rich content right (desktop). Mobile: stacked.
  const grid: CSSProperties = isDesktop
    ? { display: "grid", gridTemplateColumns: "minmax(0, 1.25fr) minmax(360px, 1fr)" }
    : { display: "grid", gridTemplateRows: "auto 1fr" };

  // Right content uses full height scroll on desktop
  const right: CSSProperties = isDesktop
    ? { padding: 16, borderLeft: `1px solid ${COLOR.line}`, overflowY: "auto", background: "#fff" }
    : { padding: 16, overflowY: "auto", background: "#fff" };

  const kpis = (step?.specs ?? []).slice(0, 3).concat((step?.benefits ?? []).slice(0, 1));
        ...
        <div style={grid}>
          ...
          >
            ...
            {isVideo ? (
              ...
              <SafeImage src={step.url} alt={step.title} fit="contain" aspect={16 / 9} />
            )}

            {/* Arrows */}
            {steps.length > 1 && (
              <>
                <button aria-label="Previous" onClick={() => setIdx((p) => (p - 1 + steps.length) % steps.length)}
                        style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", ...btn, background: COLOR.ghost }}>‹</button>
                <button aria-label="Next" onClick={() => setIdx((p) => (p + 1) % steps.length)}
                        style={{ position: "absolute", right: 16, top: "50%", transform: "translateY(-50%)", ...btn, background: COLOR.ghost }}>›</button>
              </>
            )}
              ...
              <div style={{ position: "absolute", bottom: 12, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 8 }}>
                {steps.map((_, i) => (
                  <button key={i} onClick={() => setIdx(i)} aria-label={`Go to step ${i + 1}`}
                          style={{ width: 8, height: 8, borderRadius: 999, border: 0, background: i === idx ? COLOR.brand : "#D1D5DB" }} />
                ))}
              </div>
            )}

            {/* Thumb rail (desktop): vertical on the far right of media */}
            {isDesktop && steps.length > 1 && (
              <div style={{ position: "absolute", right: 12, top: 12, display: "flex", flexDirection: "column", gap: 8 }}>
                {steps.map((s, i) => (
                  <button key={i} onClick={() => setIdx(i)} aria-label={`Step ${i + 1}`}
                          style={{ width: 72, height: 48, borderRadius: 8, overflow: "hidden", border: i === idx ? `2px solid ${COLOR.brand}` : `1px solid ${COLOR.line}`, background: "#fff" }}>
                    {s.video ? (
          ...
          <div style={right}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12 }}>
              ...
              <div style={{ color: COLOR.subt, fontSize: 13 }}>{item.category || ""}</div>
            </div>

            {/* KPI Chips */}
            {kpis.length > 0 && (
              <div style={{ marginTop: 12, display: "flex", flexWrap: "wrap", gap: 8 }}>
                {kpis.map((k, i) => <StatPill key={i} label={k} />)}
              </div>
            )}
            ...
            {step.body && (
              <div style={{ ...card, padding: 16, marginTop: 16 }}>
                ...
                <p style={{ color: COLOR.subt, lineHeight: 1.6 }}>{step.body}</p>
              </div>
            )}
            ...
            <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              <BulletBlock title="Benefits" items={step.benefits} dotColor="#10B981" />
              <BulletBlock title="Technology" items={step.technology} dotColor="#3B82F6" />
            </div>

            {/* Quick step jump (desktop inside right; mobile at bottom) */}
            {!isDesktop && steps.length > 1 && (
              <div style={{ marginTop: 16 }}>
                <div style={{ fontWeight: 800, marginBottom: 10 }}>Explore steps</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 8 }}>
                  {steps.map((s, i) => (
                    <button key={i} onClick={() => setIdx(i)} aria-label={`Step ${i + 1}`}
                            style={{ ...card, overflow: "hidden", border: i === idx ? `2px solid ${COLOR.brand}` : `1px solid ${COLOR.line}` }}>
                      {s.video ? (
                        <div style={{ aspectRatio: "16/9", display: "grid", placeItems: "center", fontSize: 12 }}>Video</div>
                      ) : (
                        <img src={s.url} alt={s.title} style={{ width: "100%", aspectRatio: "16/9", objectFit: "cover" }} />
                      )}
                    </button>
                  ))}
                </div>
            ...
            {steps.length > 1 && (
              <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
                <button style={btn} onClick={() => setIdx((p) => (p - 1 + steps.length) % steps.length)}>Previous</button>
                <button style={btn} onClick={() => setIdx((p) => (p + 1) % steps.length)}>Next</button>
              </div>
            )}
          </div>
        </div>

        {/* Mobile bottom safe controls */}
        {!isDesktop && steps.length > 1 && (
          <div style={{ padding: 12, borderTop: `1px solid ${COLOR.line}`, background: "#fff", display: "flex", justifyContent: "space-between", gap: 8 }}>
            <button style={btn} onClick={() => setIdx((p) => (p - 1 + steps.length) % steps.length)}>‹ Prev</button>
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              {steps.map((_, i) => <span key={i} style={{ width: 8, height: 8, borderRadius: 999, background: i === idx ? COLOR.brand : "#D1D5DB" }} />)}
            </div>
            <button style={btn} onClick={() => setIdx((p) => (p + 1) % steps.length)}>Next ›</button>
          </div>
        )}
...
/* ----- Page scaffold (Wistia hero + cards) ----- */
const VehicleMediaShowcase: React.FC<Props> = ({ vehicle, wistiaMediaId = "kvdhnonllm" }) => {
  ...
  const h2: CSSProperties = { fontSize: isDesktop ? 44 : 28, fontWeight: 900, letterSpacing: -0.5, color: COLOR.ink };
    ...
    <section aria-label="Media Studio">
      <div style={wrap}>
        <div style={header}>
          ...
          <p style={{ color: COLOR.subt, marginTop: 6 }}>Click any card to open a **rich** guided journey. Video plays above.</p>
        </div>
      ...
      <JourneyModal open={!!journeyOf} item={journeyOf} onClose={() => setJourneyOf(undefined)} />
    </section>
  );
};
