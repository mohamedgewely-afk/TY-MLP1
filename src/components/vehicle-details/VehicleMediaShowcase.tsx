...
/* ================= Variant content: Unique Components ================= */
...
const PerformancePanel: React.FC<{ slide: Slide | null; item: MediaItem }> = ({ slide, item }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="pt-2">
        <div className="h-24 bg-red-50/70 rounded-xl flex items-center justify-center p-4">
          <div className="relative h-full w-full">
            <div className="h-full w-full rounded-full border-4 border-red-600 animate-spin-slow opacity-20" />
            <div className="absolute inset-0 grid place-items-center font-bold text-red-800">Torque Curve</div>
          </div>
        </div>
      </div>
    </div>
...
const SafetyPanel: React.FC<{ slide: Slide | null; item: MediaItem }> = ({ slide, item }) => {
  ...
  const [activeFeature, setActiveFeature] = useState<string | null>(adas[0] || null);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-4 bg-zinc-50 rounded-xl">
        <ImageSafe src="https://i.imgur.com/example-car.png" alt="Car safety diagram" className="w-full h-auto mb-4" />
          ...
          {adas.map((feature, i) => (
            <button
              ...
              className={cx("flex items-center gap-3 p-3 rounded-lg border", 
                activeFeature === feature ? "bg-blue-100 border-blue-500" : "bg-white border-zinc-200")}
            >
              <span className={cx("h-5 w-5 rounded-full", activeFeature === feature ? "bg-blue-700" : "bg-zinc-300")} />
              <span className="text-sm font-medium">{feature}</span>
            </button>
          ))}
        </div>
        {activeFeature && (
          <p className="mt-4 p-3 rounded-lg bg-blue-50 text-blue-800 text-sm">{`Details for: ${activeFeature}`}</p>
        )}
      </div>
    </div>
  );
...
// Router to select which panel to show
      ...
      {open &&
        ReactDOM.createPortal(
          ...
          >
            ...
            >
              {/* Header: simplified for mobile, consistent title/category */}
              ...
              <div className="flex-1 overflow-y-auto grid grid-rows-[minmax(0,56svh)_minmax(0,1fr)] md:grid-cols-[minmax(0,7fr)_minmax(0,5fr)] md:grid-rows-1">
                ...
                <div className="flex flex-col bg-white md:rounded-r-2xl min-h-0">
                    ...
                    <div className="sticky bottom-0 z-20 flex items-center justify-between border-t p-4 bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
                        ...
                        <div className={cx(total <= 1 ? "w-full" : "ml-auto flex gap-3")}>
                            <button
                                onClick={openBooking}
                                className={cx(
                            ...
                            <button 
                                className="hidden md:inline-flex rounded-full border px-4 py-2 text-sm hover:bg-zinc-50" 
                                onClick={() => setOpen(null)}>
                                Close
                            </button>
                        </div>
