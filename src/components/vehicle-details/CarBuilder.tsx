...
const CarBuilder: React.FC<CarBuilderProps> = ({ vehicle, isOpen, onClose }) => {
  ...
  const calculateTotalPrice = useCallback(() => {
    ...
    const exteriorPricing = {
      "pearl white": 0, "midnight black": 500, "silver metallic": 300, "deep blue": 400, "ruby red": 600,
  ...
  }, [config, vehicle.price, normalize]);
  ...
  const sharedProps = useMemo(
    () => ({
      ...
      calculateTotalPrice, handlePayment, goBack, goNext, onClose, onReset: () => setShowResetDialog(true),
    }),
    [vehicle, step, config, showConfirmation, calculateTotalPrice, handlePayment, goBack, goNext, onClose]
  );

  if (!isInitialized) {
    const Shell = isMobile ? MobileDialog : Dialog;
    const ShellContent = isMobile ? MobileDialogContent : DialogContent;
    return (
      <Shell open={isOpen} onOpenChange={onClose}>
        <ShellContent className="p-0 w-screen h-screen grid place-items-center">
          <VisuallyHidden>
            <DialogTitle>Loading Car Builder</DialogTitle>
            <DialogDescription>Initializing…</DialogDescription>
          </VisuallyHidden>
          <div className="animate-spin h-8 w-8 rounded-full border-2 border-primary border-b-transparent" />
        </ShellContent>
      </Shell>
    );
  }
    ...
    <>
      <AlertDialog open={showResetDialog} onOpenChange={setShowResetDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This will clear all selections.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isResetting}>Cancel</AlertDialogCancel>
            <AlertDialogAction disabled={isResetting} onClick={handleReset} className="min-h-[44px] min-w-[44px]">
              {isResetting ? (
                <>
      ...
      {isMobile ? (
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
          <DialogContent className="max-w-[96vw] h-[96vh] p-0 overflow-hidden">
            <VisuallyHidden>
              <DialogTitle>Build Your {vehicle.name}</DialogTitle>
              <DialogDescription>Select model year, engine, colors, grade, and accessories.</DialogDescription>
            </VisuallyHidden>
