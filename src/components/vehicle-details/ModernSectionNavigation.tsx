...
/** ========= UTIL: robust scroll-root detection (kept for container pages) ========= */
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      >
        ...
        <SideMenuNav
          sections={sections}
          headerOffset={0}          // no sticky header; zero offset
          accentColor="#EB0A1E"
          mobileTopVh={35}
...
export function SideMenuNav({
  ...
  headerOffset = 0,              // default to 0 since sticky header was removed
  accentColor = "#EB0A1E",
  mobileTopVh = 35,
  scrollRootSelector,
}: SideMenuNavProps) {
  ...
  useEffect(() => {
    ...
    // Prefer provided selector/flagged root; fallback to document
  ...
  /** Smooth scroll with header offset (0 now) */
  const jumpTo = useCallback((id: string, index: number) => {
    ...
    // If you're using a scroll container, scroll it; otherwise window
    const root = scrollRootRef.current;
    const rectTop = el.getBoundingClientRect().top + (root === document.documentElement ? window.scrollY : (root?.scrollTop || 0));
    const dest = Math.max(0, rectTop - headerOffset - 20);
