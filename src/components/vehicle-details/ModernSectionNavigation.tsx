...
// Single-file: page + SideMenuNav (no sticky header)
...
/** ========= Side Menu Nav (kept self-contained) ========= */
type SideMenuNavProps = {
  sections?: SectionItem[];
  headerOffset?: number;          // px offset if you later add a sticky header (0 now)
  accentColor?: string;           // active color
  mobileTopVh?: number;           // side tab position on mobile
  scrollRootSelector?: string;    // custom scroll container selector
};
...
export function SideMenuNav({
  ...
  /** Smooth scroll (headerOffset = 0 now) */
  const jumpTo = useCallback((id: string, index: number) => {
    ...
    const absoluteTop =
      ...
      (root === document.documentElement ? window.scrollY : (root?.scrollTop || 0));

    const dest = Math.max(0, absoluteTop - headerOffset - 20);

    if (root && root !== document.documentElement) {
      root.scrollTo({ top: dest, behavior: "smooth" });
    } else {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      >
        <SideMenuNav
          sections={sections}
          headerOffset={0}        // no sticky header
          accentColor="#EB0A1E"
          mobileTopVh={35}
        />
