...
// (Optional) If your project has this type; otherwise remove the import and the prop.
...
export interface MobileCarBuilderProps {
  ...
  onClose: () => void;
  onReset: () => void;
  deviceCategory?: DeviceCategory; // optional; remove if unused
}
...
const GRADES = ["Base", "SE", "XLE", "Limited", "Platinum"] as const;
...
const ACCESSORIES = [
  ...
  { name: "Heated Seats", price: 400, desc: "3-level front-row seat heating." },
  {
    name: "Backup Camera",
    price: 300,
...
const allowedColorsFor = (grade: string) =>
  GRADE_COLOR_MAP[grade] ?? COLORS.map((c) => c.name);
...
const APR = 0.0349;
const DOWN_PCT = 0.2;
const reserveAmount = (status: StockStatus) =>
  status === "available" ? 2000 : 5000;
...
const MobileModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-[9999]">
      ...
      <div
        className={`absolute inset-x-3 top-[10vh] rounded-2xl border border-border/30 bg-background shadow-xl ${
          className || ""
        }`}
        role="dialog"
        aria-modal="true"
      >
        {title && (
          <div className="px-4 py-3 border-b border-border/20 text-sm font-semibold">
            {title}
          </div>
        )}
        <div className="px-4 py-3 text-sm">{children}</div>
        {actions && (
          <div className="px-4 py-3 border-t border-border/20 flex items-center justify-end gap-2">
            {actions}
          </div>
        )}
      </div>
...
/* ---------- Helper: set fetchpriority via ref (Lovable-safe) ---------- */
const setFetchPriorityLow = (el: HTMLImageElement | null) => {
  if (!el) return;
  try {
...
const MobileCarBuilder: React.FC<MobileCarBuilderProps> = ({
  ...
  const [infoItem, setInfoItem] =
    useState<(typeof ACCESSORIES)[number] | null>(null);
  const [heroMode, setHeroMode] = useState<"exterior" | "interior">(
    "exterior"
  );
  ...
  const [isResetting, setIsResetting] = useState(false); // render-level gate
  ...
  useEffect(() => {
    [backRef, closeRef, exitRef].forEach(
      (r) =>
        r.current &&
        addLuxuryHapticToButton(r.current, {
          type: "luxuryPress",
          onPress: true,
        })
    );
    if (resetRef.current)
      addLuxuryHapticToButton(resetRef.current, {
        type: "premiumError",
        onPress: true,
      });
  }, []);
  ...
  const exteriorObj = useMemo(
    () => COLORS.find((c) => c.name === config.exteriorColor) || COLORS[0],
  ...
  // Preload *only current* stills; avoid preloading all colors on mobile
  useEffect(() => {
    ...
    if (interiorObj?.img) {
      ...
      (im as any).loading = "lazy";
      im.src = interiorObj.img;
    }
  }, [exteriorObj.image, interiorObj?.img, isResetting]);

  // Preload spin frames only when user opens 360 (idle; gated during reset)
  ...
  useEffect(() => {
    if (heroMode !== "exterior" && exteriorView !== "photo")
  ...
  const setGrade = useCallback(
    (g: string) => {
      ...
      setConfig((c) => {
        ...
        const nextExterior = allowed.includes(c.exteriorColor)
          ? c.exteriorColor
          : allowed[0];
        return {
          ...c,
  ...
  const setColor = useCallback(
    (name: string) => {
      ...
      setConfig((c) => ({
        ...c,
        exteriorColor: name,
        stockStatus: computeStock(c.grade, name, c.interiorColor),
      }));
    },
    [setConfig]
  );
  ...
  const setInterior = useCallback(
    (i: string) => {
      ...
      setConfig((c) => ({
        ...c,
        interiorColor: i,
        stockStatus: computeStock(c.grade, c.exteriorColor, i),
      }));
  ...
  // Helper: resume heavy work after idle / short delay
  const scheduleResume = useCallback(() => {
    if (cancelResumeRef.current) {
    ...
    >
      ...
      <div className="relative w-full bg-gradient-to-b from-muted/20 to-background border-b border-border/20 overflow-hidden">
        <div className="relative w-full h-56 sm:h-64 md:h-72 flex items-center justify-center">
          {/* Skeleton while swapping OR during reset cool-off */}
          {(!imageLoadedKey || imageLoadedKey !== heroKey || isResetting) && (
            <div
              className="absolute inset-0 m-3 rounded-xl bg-muted/20 animate-pulse"
              aria-hidden
            />
          )}
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="text-sm font-bold truncate text-foreground">
                {config.modelYear || "Year"} {vehicle.name}
              </div>
              <div className="text-[11px] text-muted-foreground truncate mt-0.5">
                {config.grade || "Select Grade"} •{" "}
                {config.engine || "Choose Engine"}
              </div>
              {config.exteriorColor && (
                <div className="text-[11px] text-primary/80 truncate mt-0.5">
                  {config.exteriorColor}
                </div>
              )}
            </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
        {/* Step 1: Year + Engine (no finance boxes here) */}
        {step === 1 && (
        ...
        {step === 2 && (
          <>
            ...
            <div className="space-y-3">
              ...
              {!config.grade ? (
                <div className="text-[12px] text-muted-foreground p-3 bg-muted/30 rounded-xl text-center">
                  Select a grade to view available colors
                </div>
              ) : (
                <>
                  <div className="text-[12px] text-muted-foreground">
                    {visibleExteriorColors.length} colors available for{" "}
                    {config.grade}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {ACCESSORIES.map((a) => {
                    ...
                    return (
                      <div
                        key={a.name}
                        className={`rounded-xl border p-2 flex items-start gap-2 ${
      ...
      {/* Footer CTA (compact) — finance info only here */}
      ...
      >
        This will clear your selections (year, engine, grade, colors and
        accessories).
      </MobileModal>
      ...
      <MobileModal
        ...
        title={infoItem?.name || "Accessory details"}
      >
        ...
        <div className="text-sm font-semibold mt-2">
          Price: AED {infoItem ? infoItem.price.toLocaleString() : 0}
        </div>
      </MobileModal>
    </motion.div>
  );
...
/* ---------- Small UI bits ---------- */
const StockBadge: React.FC<{ status: StockStatus; compact?: boolean }> = ({
  status,
  compact = false,
}) => {
  const base = compact
    ? "inline-block text-[10px] rounded-full border px-2 py-0.5"
    : "inline-block text-xs rounded-full border px-3 py-1";
  const cls =
    status === "no-stock"
