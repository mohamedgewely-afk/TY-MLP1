...
import type { StockStatus } from "./DesktopCarBuilder";
...
const MobileCarBuilder: React.FC<MobileCarBuilderProps> = ({
  vehicle, step, totalSteps, config, setConfig, calculateTotalPrice, handlePayment, goBack, goNext, onClose, onReset,
}) => {
  ...
  useEffect(() => {
    COLORS.forEach((c) => { const img = new Image(); img.src = c.image; img.decoding = "async"; (img as any).loading = "eager"; });
  }, []);
  ...
  return (
    <motion.div className="relative w-full min-h-screen flex flex-col bg-gradient-to-b from-background via-muted/10 to-background" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      {/* Header */}
      <div className="flex items-center justify-between px-3 py-2 border-b border-border/10 sticky top-0 z-30 bg-background/90 backdrop-blur">
        <div className="flex items-center gap-1.5">
          <button ref={step > 1 ? backRef : closeRef} onClick={() => (step > 1 ? goBack() : onClose())} className="rounded-xl border p-2.5" aria-label={step > 1 ? "Back" : "Close"}>
      ...
      <div className="relative w-full h-40 border-b border-border/10">
        <motion.img key={`${activeImg}-${config.grade}-${config.modelYear}`} src={activeImg} alt={`${config.exteriorColor} ${vehicle.name}`} className="absolute inset-0 w-full h-full object-contain" initial={{ opacity: 0, scale: 1.04 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: "spring", stiffness: 240, damping: 24 }} decoding="async" loading="eager" />
        <div className="absolute bottom-2 left-2 right-2 px-3 py-2 rounded-2xl backdrop-blur-xl border border-white/10 bg-background/80">
          <div className="flex items-center justify-between gap-2">
            <div className="min-w-0">
              ...
              <div className="text-[11px] text-muted-foreground truncate">{config.grade || "Select Grade"} · {config.engine || "Choose Engine"}</div>
            </div>
          ...
          <>
            <div>
              ...
              <div className="flex items-center gap-2">
                {YEARS.map((y) => (
                  <button key={y} onClick={() => setYear(y)} className={`rounded-full border px-3 py-1.5 text-xs ${config.modelYear === y ? "border-primary/60 bg-primary/5" : "border-border/60"}`}>{y}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-2">
                {ENGINES.map((e) => (
                  <button key={e} onClick={() => setEngine(e)} className={`rounded-full border px-3 py-1.5 text-xs ${config.engine === e ? "border-primary/60 bg-primary/5" : "border-border/60"}`}>{e}</button>
                ))}
              </div>
            </div>
          </>
          ...
          <>
            ...
            <div>
              ...
              <div className="flex items-center gap-3 overflow-x-auto pb-2">
                {COLORS.map((c) => (
                  <button key={c.name} onClick={() => setColor(c.name)} className={`shrink-0 w-11 h-11 rounded-full border relative ${config.exteriorColor === c.name ? "border-primary ring-2 ring-primary/30" : "border-border/60"}`} aria-label={c.name}>
            ...
            <div>
              ...
              <div className="text-xs rounded-full border px-3 py-1 inline-block
                ${config.stockStatus === "no-stock" ? "border-destructive/30 text-destructive bg-destructive/10" :
                  config.stockStatus === "pipeline" ? "border-amber-500/30 text-amber-700 bg-amber-500/10" :
                  "border-emerald-500/30 text-emerald-700 bg-emerald-500/10"}`}>
                {config.stockStatus === "no-stock" ? "No stock" : config.stockStatus === "pipeline" ? "Pipeline stock" : "Available"}
              </div>
            </div>
          </>
        )}

        {/* Step 3: Confirmation (only if totalSteps === 3) */}
        {step === 3 && (
          <div className="space-y-2">
            {[
              ...
              ["Availability", config.stockStatus === "no-stock" ? "No stock" : config.stockStatus === "pipeline" ? "Pipeline stock" : "Available"],
            ].map(([l, v]) => (
              <div key={l as string} className="flex items-center justify-between border rounded-xl px-3 py-2">
      ...
      <div className="border-t border-border/10 px-3 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onContinue} disabled={disablePrimary} className="rounded-xl bg-primary text-primary-foreground px-4 py-2 text-sm font-semibold disabled:opacity-50">
              {primaryText}
            </button>
          </div>
        </div>
