...
/* ─────────────────────────────────────────────────────────
   GR MODE HOOK (prop + URL + localStorage)
   ───────────────────────────────────────────────────────── */
function useGRMode(defaultProp?: boolean) {
  const getInitial = () => {
    if (typeof window !== "undefined") {
      const url = new URLSearchParams(window.location.search).get("gr");
  ...
  const [isGR, setIsGR] = React.useState<boolean>(getInitial);

  // keep prop in sync if provided
  React.useEffect(() => {
    if (typeof defaultProp === "boolean") setIsGR(defaultProp);
  ...
  const toggleGR = React.useCallback(() => setIsGR((v) => !v), []);
...
const quickActionCards: QuickMenuItem[] = [
  {
    id: "test-drive",
...
const MobileStickyNav: React.FC<MobileStickyNavProps> = ({
  ...
  onCarBuilder,
  onFinanceCalculator,
  grMode, // optional external start in GR
}) => {
  ...
  // GR mode state via hook
  const { isGR, toggleGR } = useGRMode(grMode);

  // track if user changed the category (so GR doesn’t override later)
  ...
  // Default to Performance when GR turns on (first time only)
  useEffect(() => {
    ...
    <>
      ...
      {debugVisible && (
        <div className="fixed top-0 left-0 right-0 bg-red-500 text-white text-xs p-2 z-[9999]">
          <div className="text-center font-mono">
            ...
            {isMobile ? "✅" : "❌"} | Forced: {forceVisible ? "✅" : "❌"} | GR: {isGR ? "ON" : "OFF"}
          </div>
        </div>
      )}
        ...
        {isActionsExpanded && vehicle && (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className={cn("font-bold", isGR ? "text-white tracking-wide" : "text-gray-900")}>{vehicle.name}</h3>
                <span className={cn("text-lg font-bold", isGR ? "text-red-400" : "text-primary")}>AED {fmt.format(vehicle.price)}</span>
              </div>
              <Button
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            {/* Header */}
            <div
              className={cn("flex items-center justify-between p-4 border-b", isGR ? "border-neutral-800" : "")}
              style={isGR ? carbonStyle : undefined}
            >
              <div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={cn(
                    ...
                    isGR
                      ...
                      : "bg-gray-200/60 text-gray-700 dark:bg-gray-800 dark:text-gray-200"
                  )}
                  title="GR Mode"
                >
                  {isGR && (
                    <span
                      aria-hidden
                      className="absolute left-1 top-1/2 -translate-y-1/2 h-4 w-6 rounded-sm"
                      style={{ backgroundImage: CHECKER_URI, backgroundSize: "auto 100%" }}
                    />
                  )}
                  <span className={cn("ml-6", !isGR && "ml-0")}>GR</span>
                </button>
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isGR ? "text-white hover:bg-white/10" : "text-red-600 hover:bg-red-100 dark:hover:bg-red-900"
                  )}
            ...
            <div className="overflow-y-auto max-h-[calc(75vh-100px)]">
                              ...
                              >
                                <CardContent className={cn("flex flex-col justify-between h-full p-4", isGR ? "text-white" : "")}>
                                  <div className="flex items-start justify-between">
                                    ...
                                    <div className={cn(isGR ? "opacity-95" : "opacity-80")}>{card.icon}</div>
                                  </div>
                            ...
                            <motion.button
                              ...
                              className={cn(
                                ...
                                selectedCategory === category.id
                                  ? isGR
                                    ? "text-white shadow-lg scale-105 border border-red-700"
                                    : "bg-toyota-red text-white shadow-lg scale-105"
                                  : isGR
                                    ? "text-neutral-200 border border-neutral-800 hover:bg-neutral-900"
                            ...
                            >
                              ...
                              >
                                <div
                                  className={cn(
                                    "aspect-[16/10] w-full",
                                    isGR ? "relative" : "bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700"
                                  )}
                                  style={isGR ? carbonStyle : undefined}
                                >
                                  {v.image && (
                                  ...
                                  {isGR && <div className="absolute -left-10 -top-10 h-20 w-40 rotate-[-15deg] opacity-90" style={{ background: GR_GRADIENT }} />}
                                </div>
                                <CardContent className="p-4">
                                  ...
                                  <div className="flex justify-between items-center">
                                    ...
                                    <span className={cn("text-sm font-semibold flex items-center", isGR ? "text-red-400" : "text-toyota-red")}>
                          ...
                          {searchResults.map((v) => (
                            <CarouselItem key={v.name} className="basis-2/3 pl-4">
                              <Link
                                ...
                                aria-label={`View ${v.name}`}
                              >
                                <Card className={cn("h-24 overflow-hidden hover:shadow-lg transition-shadow", isGR ? "border border-neutral-800 bg-neutral-950" : "")}>
                                  <CardContent className="flex items-center h-full p-4">
                                    <div
                                      className={cn("w-16 h-12 rounded-lg mr-3 flex-shrink-0", isGR ? "overflow-hidden" : "bg-gray-100 dark:bg-gray-800")}
                                      style={isGR ? carbonStyle : undefined}
                                    >
                                      {v.image && (
                                        <img
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <h3 className={cn("font-medium text-sm truncate", isGR ? "text-white" : "text-gray-900 dark:text-gray-100")}>{v.name}</h3>
                                      <p className={cn("text-xs", isGR ? "text-neutral-400" : "text-gray-500 dark:text-gray-400")}>AED {fmt.format(v.price)}</p>
                                    </div>
              ...
              {activeSection === "pre-owned" && (
                <motion.div className="p-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                  <h4 className={cn("text-lg font-semibold mb-4", isGR ? "text-neutral-200" : "text-gray-800 dark:text-gray-200")}>Pre-Owned Vehicles</h4>
                  ...
                  <div className={cn("mb-6 p-4 rounded-xl", isGR ? "bg-neutral-950 border border-neutral-800" : "bg-gray-50 dark:bg-gray-800")}>
                    ...
                    <div className="flex items-center space-x-3">
                      <Sliders className={cn("h-5 w-5")} style={isGR ? { color: GR_RED } : { color: "#CC0000" }} />
                      <input
                        type="range"
                        min={30000}
                        max={300000}
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                <div
                                  className={cn(
                                    "aspect-[16/10] w-full relative",
                                    isGR ? "" : "bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700"
                                  )}
                                  style={isGR ? carbonStyle : undefined}
                                >
                                  <img
                                ...
                                <CardContent className="p-4">
                                  ...
                                  <div className="flex justify-between items-center mb-2">
                                    ...
                                    <p className={cn("text-xs", isGR ? "text-neutral-400" : "text-gray-500 dark:text-gray-400")}>{v.mileage}</p>
                                  </div>
                                  <div className="flex justify-between items-center">
                                    <span
                                      className={cn(
      ...
      <motion.nav
        ...
        className={cn(
          ...
          "transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] pb-safe-area-inset-bottom",
          isGR
      ...
      >
            ...
            <NavItem
              icon={
                <div className="relative">
                  ...
                  <div className="relative rounded-full p-2" style={isGR ? { background: GR_GRADIENT } : { background: "#EF4444" }}>
              ...
              label="Actions"
              to="#"
              onClick={() => setIsActionsExpanded((v) => !v)}
        ...
        {/* Optional inline GR toggle on the bar (comment out if not needed) */}
        {/* <div className="absolute right-2 bottom-2 sm:right-3 sm:bottom-3">
          <button
            ...
            className={cn(
              ...
              "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#CC0000]",
              isGR ? "bg-red-700/30 text-red-300" : "bg-gray-200/70 text-gray-700 dark:bg-gray-800 dark:text-gray-200"
            )}
    ...
    <>
      ...
      >
        <motion.div
          className={cn(
            ...
            isActive
              ? grMode
                ? "bg-red-900/40 text-red-300 scale-110"
                : "text-red-600 bg-red-50 dark:bg-red-950 scale-110"
              : grMode
                ? "text-neutral-100 bg-black/20 hover:bg-black/30"
        ...
        >
          ...
          {typeof badge === "number" && (
            <motion.div
              ...
              style={{
                background: grMode ? GR_GRADIENT : TOYOTA_GRADIENT,
          ...
          {!isScrolled && (
            <motion.span
              className={cn(
                ...
                grMode ? (isActive ? "text-red-300" : "text-neutral-300") : isActive ? "text-red-600" : "text-gray-600 dark:text-gray-400"
              )}
      ...
      {isActive && (
        <motion.div
          ...
          style={{
            background: grMode ? GR_GRADIENT : "#EB0A1E",
